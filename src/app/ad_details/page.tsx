...
export default function AdDetails() {
  ...
  const [uploadType, setUploadType] = useState<"photo" | "video">("photo"); // toggleable state
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-50 px-4 relative">
      ...
      <div className="bg-white shadow-xl rounded-xl p-7 w-full max-w-md">
          ...
          <div className="flex mb-4 justify-center rounded-full overflow-hidden">
            <button
              className={`px-4 py-2 rounded transition-all duration-300 ${
                uploadType === "photo" ? "bg-indigo-600 text-white" : "bg-gray-100"
              }`}
            ...
            <button
              className={`px-4 py-2 rounded transition-all duration-300 ${
                uploadType === "video" ? "bg-indigo-600 text-white" : "bg-gray-100"
              }`}
        ...
        {uploadType === "photo" ? (
          <>
            {preview && !isEdit && (
              <img src={preview} alt="Ad" className="w-full h-40 object-cover rounded mb-4" />
            )}
            {isEdit && (
          ...
          <>
            {preview && !isEdit && (
              <video src={preview} controls className="w-full h-40 object-cover rounded mb-4" />
            )}
            {isEdit && (
              <div className="flex justify-center">
                <label className="bg-indigo-600 text-white px-4 py-2 rounded cursor-pointer hover:bg-indigo-700 transition">
